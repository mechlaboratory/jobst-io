...
try
   ...
   data = fread(igetfield(obj, 'jobject'), totalSize, type, signed); %MOD
catch aException
...
if (numRead ~= totalSize) 
    data = localFormatData(double(data(1,1:numRead)), precision);
...
try	
    data = localFormatData(data(1), precision);   
